...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle, EFI_SYSTEM_TABLE *system_table){
    ...
    if(EFI_ERROR(status)){
        Print(L"failed to get memory map: %r", status);
        Halt();
    }
    ...
    if(EFI_ERROR(status)){
        Print(L"failed to open root directory: %r", status);
        Halt();
    }
    ...
    if(EFI_ERROR(status)){
        Print(L"failed to open file '\\kernel.elf': %r", status);
        Halt();
    }
    ...
    if(EFI_ERROR(status)){
        Print(L"failed to get file information: %r", status);
        Halt();
    }
    ...
    if(EFI_ERROR(status)){
        status = GetMemoryMap(&memmap);
        if(EFI_ERROR(status)){
            Print(L"Failed tao get memory map: %r\n", status);
            while(1);
        }
        status = gBS->ExitBootServices(image_handle, memmap.map_key);
        if(EFI_ERROR(status)){
